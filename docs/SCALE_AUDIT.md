...
| P1 | `GET /api/reports/data` | ✅ Done, including the P2 layered filtering (no denormalization): foreign-collection filter dimensions (cards, issues, history, banned_users, name index) each produce a CAPPED user-id set (truncation reported, never silent), sets intersect, and the user stream applies remaining predicates in-memory under a per-request scan budget — partial pages resume via cursor, so no request can run unbounded. No end counts by design. Legacy `userId`-field card holders joined alongside `currentUserId` |
...
| P1 | `POST /api/reports/export` | ✅ Done: N+1 replaced with batched `in` joins (30/chunk), `maxDuration: 60` (was 120 — the Hobby plan rejects anything higher at deploy time), `select()` on scans. (Date pre-filtering stays client-side: `allocationDate` mixes ISO and M/D/YYYY formats, so a string-range `where` would silently drop rows) |
...
| P2 | `cron/expire-cards` | ✅ Done: sweep queries only non-Unloaded statuses in 400-doc batches (re-query-until-empty), month claimed transactionally BEFORE sweeping so overlapping invocations can't double-process, `?dryRun=1` reports via one aggregate, `maxDuration: 60` (was 300 — the Hobby plan rejects anything higher at deploy time, so the sweep now stops at a 45s budget and resumes from a saved cursor on the next daily run) |
...
| P2 | `/reports` | ✅ Done: filters travel to the server (debounced, aborted); the client holds only the MATCHED set and re-applies predicates as a refinement — which also covers the two filters Firestore cannot express (allocation-date ranges over mixed-format strings, pending data normalization; and email/phone substrings). Option lists are static enums; exports build from the server-matched set (correct since that whole set is local); oversized filter matches show a truncation banner instead of silently missing rows |
