...
export default function NewCardModal({
  ...
  const validateRows = (): string | null => {
    ...
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      if (row.arcCardNumber.length !== 19) {
        return `Card #${i + 1}: ARC Card Number must be exactly 19 digits (got ${row.arcCardNumber.length}).`;
      }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (digits.length !== 19) {
        errors.push({
          row: rowNum,
          field: "arcCardNumber",
          message: `Must be exactly 19 digits (got ${digits.length}).`,
        });
        rowValid = false;
      }
